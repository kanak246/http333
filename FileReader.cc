...
bool FileReader::ReadFile(string *const contents) {
  ...
  // Read the File based on hw2 function and
  // make sure to check for NULL error and to
  // free the returned var after using it
  int size;
  char *f_content;
  if ((f_content = ::ReadFileToString(full_file.c_str(), &size)) == NULL) {
    return false;
  }

  // store the file contents into output param
  *contents = string(f_content, size);
  free(f_content);
  return true;
}
