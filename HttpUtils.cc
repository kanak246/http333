...
bool IsPathSafe(const string &root_dir, const string &test_file) {
  ...
  // C function that gets absolute path of a file
  // realpath()
  // 1. Plan is to use this function and check
  // if given root dir and test file is valid.
  // 2. Then I want to make sure that testFile path
  // starts with the root dir
  // 3. I want to check if testFilePath is actually in
  // root dir
  char root[PATH_MAX];
  char test_file_path[PATH_MAX];

  if (realpath(root_dir.c_str(), root) == nullptr ||
    realpath(test_file.c_str(), test_file_path) == nullptr) {
    return false;
  }
  if (strncmp(test_file_path, root, strlen(root)) != 0) {
    return false;
  }
  if (test_file_path[strlen(root)] != '/' &&
    test_file_path[strlen(root)] != '\0') {
    return false;
  }
...
string EscapeHtml(const string &from) {
  ...
  // STEP 2
  // characters needing to be replaced are
  // " -> &quot;
  // ' -> &apos;
  // < -> &lt;
  // > &gt;
  // & -> &amp;
  // I am thinking that I can use a map and then
  // if the value is in map then replace it with correct encoding
  string ret;
  std::map<char, string> escapeCode = {{'"', "&quot;"},
                                       {'\'', "&apos;"},
                                       {'<', "&lt;"},
                                       {'>', "&gt;"},
                                       {'&', "&amp;"}};
  for (char c : from) {
    if (escapeCode.find(c) != escapeCode.end()) {
        ret += escapeCode[c];
    } else {
        ret += c;
    }
